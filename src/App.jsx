...
function CourtSession({ people, onClose, onBack, colorOf }) {
  ...
  const cleanPeople = people.filter(p => p.amt === 0);
      ...
      <div style={{
        ...
        {stage === "verdict" && top && (
          <div style={{
            ...
            {hasFines && (
              <div style={{
                marginTop:14, paddingTop:14, borderTop:"1px solid rgba(232,192,116,0.25)",
                fontSize:"0.88rem", lineHeight:1.6, color:"#e8c074", fontStyle:"italic"
              }}>
                "നീ തന്നെ ശേഷൻസിലെ ഏറ്റവും വല്യ നാറി, ഇനി എങ്കിലും നന്നായിക്കൂടെ മൈരേ"
              </div>
            )}
          </div>
        )}

        {stage === "verdict" && cleanPeople.length > 0 && (
          <div style={{
            width:"100%", maxWidth:320, margin:"6px 0 22px",
            background:"rgba(122,140,63,0.12)",
            border:"2px dashed rgba(122,140,63,0.5)",
            borderRadius:16, padding:"20px 18px", textAlign:"center",
            animation:"slideUp 0.4s ease 0.15s both"
          }}>
            <div style={{ fontSize:"0.68rem", letterSpacing:2, color:"#a9c178", marginBottom:12 }}>
              🌟 ശേഷൻസിന്റെ അഭിമാനം 🌟
            </div>
            <div style={{
              display:"flex", flexWrap:"wrap", gap:10, justifyContent:"center", marginBottom:14
            }}>
              {cleanPeople.map(p => (
                <div key={p.id} style={{ display:"flex", flexDirection:"column", alignItems:"center", width:60 }}>
                  {p.photo ? (
                    <img src={p.photo} alt={p.name} style={{
                      width:44, height:44, borderRadius:"50%", objectFit:"cover",
                      border:"2px solid #a9c178", marginBottom:4
                    }} />
                  ) : (
                    <div style={{ marginBottom:4 }}>
                      {p.gender === "f" ? <FemaleSVG color={colorOf(p.id)} size={40} /> : <MaleSVG color={colorOf(p.id)} size={40} />}
                    </div>
                  )}
                  <div style={{ fontSize:"0.68rem", fontWeight:600, textAlign:"center" }}>{p.name}</div>
                </div>
              ))}
            </div>
            <div style={{
              fontSize:"0.85rem", lineHeight:1.6, color:"#c8dba0", fontStyle:"italic",
              paddingTop:12, borderTop:"1px solid rgba(122,140,63,0.3)"
            }}>
              "നിങ്ങൾ ഈ ആഴ്ച ശേഷൻസിന്റെ യഥാർത്ഥ അഭിമാനമാണ്. തുടരൂ!"
            </div>
          </div>
        )}
...
// ── payments ──────────────────────────────────────────────────────────────────
function PaymentsPanel({ people, colorOf, onClose }) {
  ...
  useEffect(() => {
    const unsub = onValue(ref(db, "court/settings"), snap => {
      const data = snap.val() || {};
      setUpiId(data.upiId || "");
  ...
  async function saveCollectorSettings() {
    try {
      await set(ref(db, "court/settings"), { upiId: upiId.trim(), collectorName: collectorName.trim() });
    } catch (e) { console.error(e); }
    setEditingSettings(false);
  }

  async function markPaid(person) {
    if (!window.confirm(`Mark ₹${person.owed} from ${person.name} as paid?`)) return;
    setBusyId(person.id);
    try {
      await set(ref(db, `court/people/${person.id}/owed`), 0);
      logAction("paid", { personId: person.id, personName: person.name, amountPaid: person.owed });
    } catch (e) { console.error(e); }
    setBusyId(null);
  }

  function upiLink(person) {
    const note = encodeURIComponent(`Devika's Court fine - ${person.name}`);
    const pn = encodeURIComponent(collectorName || "Collector");
    return `upi://pay?pa=${encodeURIComponent(upiId)}&pn=${pn}&am=${person.owed}&cu=INR&tn=${note}`;
  }

  const pending = people.filter(p => (p.owed || 0) > 0).sort((a, b) => (b.owed || 0) - (a.owed || 0));
  const totalOwed = pending.reduce((s, p) => s + (p.owed || 0), 0);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
          <div style={{ fontFamily:"'Fraunces',serif", fontWeight:900, fontSize:"1.05rem" }}>💵 Payments</div>
          <button onClick={onClose} style={{
            background:"none", border:"none", color:"rgba(255,255,255,0.6)",
            fontSize:"1.1rem", cursor:"pointer", padding:0
          }}>✕</button>
        ...
        {pending.length > 0 && (
          <div style={{ fontSize:"0.78rem", color:"rgba(255,255,255,0.5)", marginBottom:14 }}>
            <strong style={{ color:"#e8c074" }}>₹{totalOwed}</strong> pending across {pending.length} {pending.length === 1 ? "person" : "people"}
          </div>
        )}

        {/* collector settings */}
        <div style={{
          background:"rgba(255,255,255,0.05)", borderRadius:10, padding:"10px 12px", marginBottom:16
        }}>
          {editingSettings ? (
            <>
              <input
                ...
                style={{
                  width:"100%", padding:"7px 10px", marginBottom:6, borderRadius:7,
              ...
              <input
                ...
                style={{
                  width:"100%", padding:"7px 10px", marginBottom:8, borderRadius:7,
                  background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.2)",
                  color:"#fff", fontSize:"0.8rem", outline:"none"
                }}
              />
              <button onClick={saveCollectorSettings} style={{
                width:"100%", padding:"7px", borderRadius:7, border:"none",
                background:"#7a8c3f", color:"#fff", fontSize:"0.78rem", fontWeight:600, cursor:"pointer"
              }}>Save</button>
            </>
          ) : (
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ fontSize:"0.76rem", color:"rgba(255,255,255,0.55)" }}>
                {upiId
                  ? <>Collector: <strong style={{ color:"#fff" }}>{collectorName || "—"}</strong> · {upiId}</>
                  : "No UPI ID set yet — pay button will be hidden"}
              </div>
              <button onClick={() => setEditingSettings(true)} style={{
                background:"none", border:"none", color:"#e8c074", fontSize:"0.72rem", cursor:"pointer", flexShrink:0, marginLeft:8
              }}>Edit</button>
            </div>
          )}
        </div>
          ...
          {pending.length === 0 ? (
            ...
            pending.map(p => (
              <div key={p.id} style={{
                ...
                {p.photo ? (
                  <img src={p.photo} alt="" style={{ width:34, height:34, borderRadius:"50%", objectFit:"cover", flexShrink:0 }} />
                ) : (
                  <div style={{
                    width:34, height:34, borderRadius:"50%", flexShrink:0,
                    background:colorOf(p.id), display:"flex", alignItems:"center", justifyContent:"center",
                    fontSize:"0.85rem", fontWeight:700
                  }}>{p.name[0]?.toUpperCase()}</div>
                )}
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:"0.88rem", fontWeight:600 }}>{p.name}</div>
                  <div style={{ fontFamily:"'Fraunces',serif", fontWeight:900, fontSize:"0.95rem", color:"#e8c074" }}>₹{p.owed}</div>
                </div>
                <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                  {upiId && (
                    <a href={upiLink(p)} style={{
                      padding:"6px 10px", borderRadius:7, textDecoration:"none",
                      border:"1px solid rgba(122,140,63,0.4)", background:"rgba(122,140,63,0.15)",
                      color:"#a9c178", fontSize:"0.68rem", fontWeight:600
                    }}>Pay via UPI</a>
                  )}
                  <button onClick={() => markPaid(p)} disabled={busyId === p.id} style={{
                    padding:"6px 10px", borderRadius:7,
                    border:"1px solid rgba(232,192,116,0.35)", background:"rgba(232,192,116,0.1)",
                    color:"#e8c074", fontSize:"0.68rem", cursor:"pointer", fontWeight:600
                  }}>{busyId === p.id ? "…" : "Mark Paid"}</button>
                </div>
              </div>
            ))
          )}
...
export default function App() {
  ...
  async function closeCourt() {
    ...
    // resets everyone's fine to ₹0 for the new week — but first, whatever
    // they hadn't paid gets added to their persistent "owed" balance, so
    // debt isn't quietly forgotten just because the jar reset.
    ...
    setPeople(prev => prev.map(p => ({ ...p, amt: 0, owed: p.amt > 0 ? (p.owed || 0) + p.amt : p.owed })));
  ...
  const totalPot = people.reduce((s, p) => s + p.amt, 0);
  const pendingCount = people.filter(p => (p.owed || 0) > 0).length;
  ...
  return (
    <>
      ...
      <div style={{
        ...
        }}>പെഴച്ച വർത്താനം പടിക്ക് പുറത്ത്</div>
        ...
        {/* action buttons */}
        <div style={{ display:"flex", gap:10, marginBottom:totalPot > 0 ? 16 : 22, flexWrap:"wrap", justifyContent:"center" }}>
          ...
          <button onClick={() => setShowPayments(true)} style={{
            padding:"8px 18px", borderRadius:22, cursor:"pointer",
            border:"1px solid rgba(255,255,255,0.32)",
            background:"rgba(255,255,255,0.15)", color:"#fff",
            fontSize:"0.8rem", fontWeight:500, backdropFilter:"blur(4px)",
      ...
      {showPayments && (
        <PaymentsPanel people={people} colorOf={colorOf} onClose={() => setShowPayments(false)} />
      )}
      {showChat && (
